...
```bash
# start.sh を実行するだけで全プロセスが起動する
./start.sh

# 内部では以下が順番に実行される
# 1. ollama serve（Ollamaサーバー起動）
...
| 「← 戻る」 | 途中離脱。確認ダイアログを表示し、OKでSQLiteにstatus='abandoned'で保存してS01へ |
...
| 主張文 | LLMが全ターンのユーザー発言を要約して生成。コピーボタンでクリップボードにコピー可能 |
| 論理マップ | Mermaidグラフで描画。「全画面で見る」でモーダル拡大表示 |
...
| 1.1 | 2026-03-18 | §4 | 修正 | S02セッション画面の会話エリアから不要な文言「テーマについて話してください」を削除。S03結果画面の主張文エリアのラベルを「磨き上げられた主張文」から「あなたの結論」に変更。 |